...
-- Schema Libri
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `Libri` DEFAULT CHARACTER SET utf8 ;
-- -----------------------------------------------------
-- Schema world
-- -----------------------------------------------------
...
-- Table `Libri`.`utenti`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `Libri`.`utenti` ;

CREATE TABLE IF NOT EXISTS `Libri`.`utenti` (
  `nome` VARCHAR(255) NULL,
  `cognome` VARCHAR(45) NULL,
  `email` VARCHAR(45) NULL,
  `indirizzo` VARCHAR(45) NULL,
  `codice_fiscale` VARCHAR(16) NULL,
  `utenticol` VARCHAR(45) NULL,
...
-- Table `Libri`.`Libri`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `Libri`.`Libri` ;

CREATE TABLE IF NOT EXISTS `Libri`.`Libri` (
  `idLibri` INT NOT NULL AUTO_INCREMENT,
  `titolo` VARCHAR(45) NULL,
  `casa_editrice` VARCHAR(45) NULL,
  `genere` VARCHAR(45) NULL,
  `codice_libro` VARCHAR(46) NULL,
  `auttore` VARCHAR(16) NULL,
  `datauscita` VARCHAR(45) NULL,
  UNIQUE INDEX `codice_libro_UNIQUE` (`codice_libro` ASC) VISIBLE,
  PRIMARY KEY (`idLibri`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `Libri`.`Prestiti`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `Libri`.`Prestiti` ;

CREATE TABLE IF NOT EXISTS `Libri`.`Prestiti` (
  `prestito_id` INT NOT NULL,
  `inizio_prestito` VARCHAR(45) NOT NULL,
  `fine_prestito` VARCHAR(45) NULL,
  `utenti_utenticol` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`prestito_id`, `utenti_utenticol`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `Libri`.`prestiti` (
  ...
  `Libri_idLibri` INT NOT NULL,
  `id` INT NOT NULL AUTO_INCREMENT,
  `inizioprestito` VARCHAR(45) NOT NULL,
  `utenti_has_Libricol` DATETIME(20) NOT NULL,
  `fineprestito` DATETIME(20) NOT NULL,
  INDEX `fk_utenti_has_Libri_Libri1_idx` (`Libri_idLibri` ASC) VISIBLE,
  INDEX `fk_utenti_has_Libri_utenti_idx` (`utenti_idutenti` ASC) VISIBLE,
  PRIMARY KEY (`id`))
ENGINE = InnoDB;
...
USE `sakila`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `sakila`.`nicer_but_slower_film_list` AS select `sakila`.`film`.`film_id` AS `FID`,`sakila`.`film`.`title` AS `title`,`sakila`.`film`.`description` AS `description`,`sakila`.`category`.`name` AS `category`,`sakila`.`film`.`rental_rate` AS `price`,`sakila`.`film`.`length` AS `length`,`sakila`.`film`.`rating` AS `rating`,group_concat(concat(concat(upper(substr(`sakila`.`actor`.`first_name`,1,1)),lower(substr(`sakila`.`actor`.`first_name`,2,length(`sakila`.`actor`.`first_name`))),_utf8mb4' ',concat(upper(substr(`sakila`.`actor`.`last_name`,1,1)),lower(substr(`sakila`.`actor`.`last_name`,2,length(`sakila`.`actor`.`last_name`)))))) separator ', ') AS `actors` from ((((`sakila`.`film` left join `sakila`.`film_category` on((`sakila`.`film_category`.`film_id` = `sakila`.`film`.`film_id`))) left join `sakila`.`category` on((`sakila`.`category`.`category_id` = `sakila`.`film_category`.`category_id`))) left join `sakila`.`film_actor` on((`sakila`.`film`.`film_id` = `sakila`.`film_actor`.`film_id`))) left join `sakila`.`actor` on((`sakila`.`film_actor`.`actor_id` = `sakila`.`actor`.`actor_id`))) group by `sakila`.`film`.`film_id`,`sakila`.`category`.`name`;
